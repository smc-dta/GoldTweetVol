...
#daily tweet volumes and daily returns
...
subset <- data[data$tw < 250, ]
plot(subset$r, subset$tw, main = "Daily Gold Price Returns vs. Daily Tweet Volume", 
     xlab = "Return", ylab = "Tweets")

#plotting daily tweet volumes and daily volatility
plot(data$ar, data$tw, main = "Daily Gold Price Returns vs. Daily Tweet Volatility",
     xlab = "Return", ylab = "Tweets")
#plotting the section where number of tweets is less than 250
plot(subset$ar, subset$tw, main = "Daily Gold Price Returns vs. Daily Tweet Volatility", 
     xlab = "Return", ylab = "Tweets")


#dataset is split into two groups, data1 and data2, due to a large gap in time series data.
